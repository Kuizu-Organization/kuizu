...
const FoldersPage = () => {
    ...
    const fetchData = async () => {
        try {
            setIsLoading(true);
            const promises = [getPublicFolders()];
            if (isAuthenticated) {
                promises.push(getMyFolders());
            }

            const results = await Promise.all(promises);
            setPublicFolders(results[0]);
            if (isAuthenticated) {
                setFolders(results[1]);
            }
        } catch (error) {
    ...
    return (
        <div className="folders-container">
            ...
            {isAuthenticated && (
                <>
                    ...
                    {folders.length > 0 ? (
                        ...
                        <div className="folders-empty" style={{ marginBottom: '60px' }}>
            ...
            {publicFolders.length > 0 && (
                <div className="public-folders-section" style={!isAuthenticated ? { marginTop: 0 } : {}}>
                    <div className="public-folders-header">
                        <Globe size={20} />
                        <h2>{isAuthenticated ? 'Suggested Folders' : 'Public Folders'}</h2>
